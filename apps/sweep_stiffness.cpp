...
int main() {
    constexpr std::array<double, 3> stiffnesses{100.0, 200.0, 400.0};
    ...
    std::array<atj::JointModel, 3> models{
        atj::JointModel(atj::Parameters{.inertia = inertia,
                                        .damping = damping,
                                        .spring_stiffness = stiffnesses[0],
        ...
        atj::JointModel(atj::Parameters{.inertia = inertia,
                                        .damping = damping,
                                        .spring_stiffness = stiffnesses[1],
                                        .joint_radius = joint_radius,
                                        .motor_radius = motor_radius,
                                        .preload = preload}),
        ...
        atj::JointModel(atj::Parameters{.inertia = inertia,
                                        .damping = damping,
                                        .spring_stiffness = stiffnesses[2],
                                        .joint_radius = joint_radius,
                                        .motor_radius = motor_radius,
                                        .preload = preload})};
    ...
    std::ofstream output("results/stiffness_sweep.csv");
    ...
    std::cout << "Stiffness sweep complete.\n\n";

    for (std::size_t i = 0; i < stiffnesses.size(); ++i) {
        std::cout << "k = " << stiffnesses[i] << " N/m\n"
                  << "  peak q:    " << peak_q[i] << " rad\n"
